...
class Comic < ActiveRecord::Base
  ...
  def self.create_url
    ...
    url = "http://gateway.marvel.com:80/v1/public/comics?format=comic&formatType=comic&noVariants=true&dateDescriptor=nextWeek&ts=#{ts}&apikey=#{public_key}&hash=#{hash}"
  end

  def self.next_week(url)
    doc = JSON.parse(open(url).read)
    next_week = Week.create(:week_number => Date.today.cweek + 1)
    if doc['code'] == 200
      ...
      results.each do |comic|
        ...
        new_comic = Comic.create(:title => comic['title'], :issue_number => comic['issueNumber'], :description => comic['description'], :marvel_id => comic['id'], :week_id => next_week.id )
        if new_comic.save
